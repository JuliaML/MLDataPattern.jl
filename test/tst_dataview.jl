@testset "ObsView" begin
    ...
    @testset "constructor" begin
        ...
        show(STDOUT, MIME"text/plain"(), ObsView(rand(2)))
    ...
    @testset "AbstractArray interface" begin
        ...
        for var in vars
            A = ObsView(var)
            @test @inferred(getobs(A)) == [getobs(var,i) for i in 1:nobs(var)]
        end
        for var in tuples
            A = ObsView(var)
            @test @inferred(getobs(A)) == [map(x->getobs(x,i), var) for i in 1:nobs(var)]
        end
        A = ObsView(X',obsdim=1)
        @test @inferred(length(A)) == 150
        @test @inferred(size(A)) == (150,)
...
@testset "BatchView" begin
    ...
    @testset "constructor" begin
        ...
        show(STDOUT, MIME"text/plain"(), BatchView(rand(2)))
